        ...
        <div className="flex items-center justify-between mb-2">
          <CollapsibleTrigger asChild>
            <Button variant="ghost" size="sm" className="!px-0 hover:bg-transparent">
              <ChevronDown
                className={`h-4 w-4 transition-transform ${isAttachmentsOpen ? 'transform rotate-180' : ''}`}
              />
              <span className="text-sm font-semibold ml-2">Files</span>
              <span className="text-sm text-muted-foreground ml-2">({attachments.length})</span>
            </Button>
          </CollapsibleTrigger>
        </div>
        <CollapsibleContent>
