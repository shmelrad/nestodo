      ...
      queryClient.setQueryData(['board', boardId], (oldData: Board) => {
        const updatedTaskLists = oldData.taskLists.map((taskList) => {
          if (taskList.id === task.taskListId) {
            return {
              ...taskList,
              tasks: taskList.tasks.map((t) => (t.id === task.id ? updatedTask : t)),
            }
          }
          return taskList
        })
  ...
  return (
    <>
      <div
        ...
        } flex flex-col relative ${task.priority ? `border-l-4 ${priorityColors[task.priority]}` : ''}`}
        ref={setNodeRef}
        style={style}
        {...attributes}
        {...listeners}
      ...
      >
        <div className="flex flex-col">
          ...
          <div className="flex items-center">
            ...
            <div
              className={`flex flex-col gap-2 transition-all duration-300 ease-in-out ${isMouseOver && !isDraggingTask ? 'ml-6' : 'ml-0'}`}
            >
              <div className="flex items-start justify-between">
                <h3
                  className={`${task.completed ? 'text-muted-foreground line-through' : ''} text-sm whitespace-pre-wrap [overflow-wrap:anywhere] ${isDragging ? 'invisible' : ''}`}
                >
                  {task.title}
                </h3>
              </div>
